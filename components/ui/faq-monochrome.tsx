...
/**
 * CSS animations for the FAQ intro pill.
 * Injected once via a <style> element (static content, not user input).
 */
const FAQ_ANIMATIONS_CSS = `
...
.faq1-intro {
  ...
  color: rgba(248,250,252,0.92); text-transform: uppercase;
  letter-spacing: 0.35em; font-size: 0.65rem; width: 100%; max-width: 24rem;
  margin: 0 auto; mix-blend-mode: screen; opacity: 0;
  transform: translate3d(0,12px,0); filter: blur(8px);
  transition: opacity 720ms ease, transform 720ms ease, filter 720ms ease;
  isolation: isolate;
}
.faq1-intro--light {
  border-color: rgba(17,17,17,0.12); background: rgba(248,250,252,0.88);
  color: rgba(15,23,42,0.78); mix-blend-mode: multiply;
}
...
.faq1-intro__beam {
  ...
  animation: faq1-beam-spin 18s linear infinite; opacity: 0.55;
}
...
.faq1-intro__meter {
  ...
  animation: faq1-meter 5.8s ease-in-out infinite; opacity: 0.7;
}
.faq1-intro__tick {
...
export function FaqMonochrome({
  ...
  subtitle = "Everything you need to know, condensed into calm clarity.",
  items,
  className,
}: FaqMonochromeProps) {
  ...
  // Inject animations stylesheet (static CSS, safe content)
  useEffect(() => {
    if (typeof document === "undefined") return;
    if (document.getElementById(INTRO_STYLE_ID)) return;
    const style = document.createElement("style");
    style.id = INTRO_STYLE_ID;
    style.textContent = FAQ_ANIMATIONS_CSS;
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "relative z-10 mx-auto flex max-w-4xl flex-col gap-12 px-6 py-20 md:py-28 lg:max-w-5xl lg:px-12",
      ...
      >
        ...
        <header className="space-y-4 text-center">
          ...
          <p className={cn("mx-auto max-w-xl text-base", palette.muted)}>
            {subtitle}
          </p>
        </header>
          ...
          {items.map((item, index) => {
            ...
            return (
              <li
                key={item.question}
                className={cn(
                  "group relative overflow-hidden rounded-3xl border backdrop-blur-xl transition-all duration-500 hover:-translate-y-0.5 focus-within:-translate-y-0.5",
                  palette.border,
                  palette.panel,
                  palette.shadow,
                )}
              ...
              >
                ...
                <button
                  ...
                  className="relative flex w-full items-start gap-6 px-8 py-7 text-left transition-colors duration-300 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4"
                >
                  <span
                    className={cn(
                  ...
                  <div className="flex flex-1 flex-col gap-4">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
                      ...
                      {item.meta && (
                        <span
                          className={cn(
                            "inline-flex w-fit items-center rounded-full border px-3 py-1 text-[10px] uppercase tracking-[0.35em] transition-opacity duration-300 sm:ml-auto",
                            palette.border,
                            palette.muted,
                          )}
                        >
                    ...
                    <div
                      ...
                      className={cn(
                        "overflow-hidden text-sm leading-relaxed transition-[max-height] duration-500 ease-out",
